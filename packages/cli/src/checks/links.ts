...
export const brokenInternalLinks: Check = {
  id: 'broken-internal-links', family: 'technical-seo', evidence: 'measured', maxPoints: 8,
  async run(ctx) {
...
export const redirectHygiene: Check = {
  id: 'redirect-hygiene', family: 'security', evidence: 'measured', maxPoints: 4,
  async run(ctx) {
...
export const hreflang: Check = {
  id: 'hreflang', family: 'technical-seo', evidence: 'measured', maxPoints: 3,
  async run(ctx) {
    const pages = await pagesOf(ctx);
    if (pages.length === 0) return makeResult(this, 'fail', 'no page reachable');
