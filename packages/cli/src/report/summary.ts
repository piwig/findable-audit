...
import { axisScores, projectScore, verdictSentence, AXIS_ORDER } from './axes.js';
...
export function renderSummaryMarkdown(report: AuditReport, now: Date, lang: Lang = 'en'): string {
  const { m, axes, top, verdict, projected, host } = summaryModel(report, lang);
  const lines: string[] = [];
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .axes { display: flex; gap: .8rem; margin: 0 0 1.8rem; }
