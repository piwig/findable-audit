...
export function generateReadme(report: AuditReport, { lang }: GenerateOptions): string {
  ...
  // A34 — honest positioning: llms.txt is a near-zero-cost bet with an unproven
  // payoff (2025-26 studies measure no citation gain; ~3% adoption; Google says
  // no ranking impact). Said plainly here so an informed client is never oversold.
  const llmsCaveat = lang === 'fr'
    ? 'Transparence : `llms.txt` / `llms-full.txt` sont un pari à coût quasi nul mais au bénéfice incertain — aucune étude 2025-26 ne mesure de gain de citation et Google annonce un impact nul sur le classement. Publiez-les parce qu\'ils ne coûtent rien, pas en attendant un effet garanti.'
    : 'Transparency: `llms.txt` / `llms-full.txt` are a near-zero-cost bet with an unproven payoff — 2025-26 studies measure no citation gain and Google states they have no ranking impact. Ship them because they cost nothing, not because an effect is guaranteed.';
