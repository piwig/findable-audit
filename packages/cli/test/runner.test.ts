...
describe('runAudit', () => {
  ...
  it('produces a normalized score over non-skipped checks', async () => {
    ...
    expect(report.results).toHaveLength(133);
    ...
    expect(skipped.map((r) => r.id).sort()).toEqual([
      ...
      'sd-breadcrumb', 'sd-faq', 'sd-graph-integrity', 'sd-localbusiness', 'sd-page-entity', 'sd-product',
      'sd-special-types', 'sd-video', 'sd-website-searchaction', 'sitemap-index-limits', 'sitemap-lastmod', 'sitemap-orphans',
      'sitemap-urls-valid', 'tls-version', 'topical-focus', 'trailing-slash', 'unique-titles', 'www-consolidation',
    ]);
  });
