...
111 checks grade against an RFC, WCAG, schema.org or a threshold Google publishes. 38 are heuristics — a ratio, a lexicon, a bar we chose. The reports badge them, so you never mistake our taste for a specification.
...
**149 checks in 8 families.** Each family scores `0–100` over its own non-skipped checks; the eight are then blended with the weights below.
...
Every check declares what its verdict rests on, and the reports say which. **110 are *measured*** — they grade against something outside this project, so two people reading the same response agree. **38 are *heuristic*** — a word count, a lexicon, a ratio, a notion of "reads like a direct answer". Reasonable people can disagree with those, and the verified research says their effectiveness varies by site, so the HTML and Markdown reports badge them and the JSON carries `evidence` on every result.
...
**How deep, compared to a general SEO audit.** Public SEO audit checklists show how the industry weighs the GEO/AI topic today: the Yvarn audit grid (47 criteria, CC-BY-NC, 2026 Q2) added "AI and LLM visibility" as a 7th category in 2026 — 4 criteria out of 47, roughly 5–10% of the score. findable-audit dedicates a whole family to it: `llm-content`, 23 checks, 18% of the weighted score. General SEO checklists are starting to notice AI visibility; here it's a first-class axis, not an addendum.

**How we compare to the audit tool every technical SEO already runs.** Since 2026 Chrome Lighthouse audits sites under its own "Agentic Browsing" category — but that category checks for one thing, the presence of `/llms.txt` (a single binary signal), and its own 2026 sample found 96.8% of sites still fail it. findable-audit's `llm-content` family covers the same ground and 22 more signals besides — content readable without JS, answer-first structure, author E-E-A-T, freshness, chunk retrieval, prompt-injection hygiene. Lighthouse is only just noticing the AI-crawler topic; here it has been a full, weighted family since the start.
...
- **Raw rule count** — SEOmator advertises 251 rules to our 149 checks.
