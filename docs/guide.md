...
### `llms-txt-lint` (2 pts)
**Verifies:** Lints an *existing* `/llms.txt` beyond its shape: exactly one H1 root, absolute link targets, same-origin links that resolve on the live site (up to 12 probed; status >= 400 counts as broken), and coherence with `sitemap.xml` (warns when none of the linked URLs appear in a published `<urlset>`). Skips when `llms.txt` is absent or unreadable — that verdict belongs to `llms-txt`.
**Why:** `llms-txt` judges the file's format; this check judges whether the file is *true*. A curated index pointing at 404s, relative paths or pages the site no longer publishes is worse than none: it hands agents dead ends with full confidence.
**Fix:** Keep `llms.txt` in sync with the site: one `# Site` root, absolute URLs, no dead links, targets present in the sitemap.
...
### `social-proof` (2 pts)
**Verifies:** On pages declaring Product, Service, LocalBusiness (any subtype) or Organization, looks for `AggregateRating` / `Review` JSON-LD — attached to the item or standalone — and checks the minimal usable shape (a rating value plus a rating/review count; an author plus a rating). Warns at worst, never fails; skips entirely when no relevant type is declared.
**Why:** Reviews shown as widgets or images are invisible to an engine assembling an answer; only the markup is read. Which pages "should" carry reviews is a judgment call, so this is an advisory heuristic — and often the cheapest high-impact markup fix for a local business. `rich-result-eligibility` grades Google's per-field requirements once the markup exists; this check asks the earlier question: is there anything there at all?
**Fix:** Expose the reviews you already collect as `AggregateRating` (`ratingValue` + `ratingCount` or `reviewCount`) on the entity customers rate, and give each `Review` an `author` and a `reviewRating`.
### `tls-version` (3 pts)
